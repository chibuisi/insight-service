...
//    @Size(max = 500, message = "email must be maximum {max} characters.")
...
//    private String email;
    ...
    @Size(max = 10, message = "availability must not exceed {max}")
...
//    @Size(max = 20, message = "topics must not exceed {max}")
//    private List<@Size(min = 2, max = 100, message = "certification(s) must be between {min} and {max} characters") String> topics;
    @NotNull(message = "topics must not be null")
    @Size(max = 50, message = "topics must not exceed {max} characters")
